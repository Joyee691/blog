...
造成内存泄漏的逻辑链：某个东西一直被引用 -> GC 认为它仍然可达 -> 无法进行回收 -> 内存泄漏
...
2. 原型 prototype 决定了**对象属性的查找路径**，当访问对象的变量时，JS 会沿着对象的 **[[prototype]]** 一层层往上找，直到 Object.prototype 为止，这个查找的过程被称之为原型链
...
>  **事件循环**是 JS 在**单线程**下还能处理 I/O、定时器、用户交互等“异步事件”的机制。
>
> 核心思路是：JS 线程只负责执行代码，**耗时操作交给宿主环境**（浏览器/Node）处理；等条件满足后，把对应的回调放进**任务队列**里。
>
> 当 JS 执行完当前这段同步代码、调用栈清空后，事件循环会从队列里取出下一个任务执行；并在每个任务结束后，先把**微任务队列**清空。
>
> 在浏览器里，通常会在“任务与任务之间”穿插渲染相关的流程（例如布局和绘制），所以如果微任务一直堆积，可能会让渲染延后。

简介：

- macro-task：宏任务tasks
  - 宏任务是事件循环中**更粗粒度的调度单位**
  - 特性：**事件循环的一轮（tick）里，只会执行一个宏任务，然后清空微任务，并且在宏任务之间浏览器通常有机会进行渲染**
  - 框架在做调度时，往往会把一些可拆分、可延后的工作放到宏任务中，从而给渲染和用户交互留出机会

- micro-task：微任务jobs
  - 微任务队列永远会在当前宏任务执行完毕后、浏览器进行渲染之前被清空
...
1. 取出一个宏任务执行（如整个 script，注意如果有多个 setTimeout 也会被分割为多个 tick 执行）
...
事件委托的前提是该事件支持冒泡，比如 focus、blur 事件就不适用
...
与 setTimeout 区别：**requestAnimationFrame 是渲染驱动**；而 **setTimeout 是时间驱动**

# Blog

一个记录学习过程的备忘录

## Javascript 相关

- [Javascript 中的柯里化（Currying）](./articles/Javascript/Currying_in_JS.md)
- [深入 ES6 的块级作用域与声明提升](./articles/Javascript/深入ES6的块级作用域与声明提升.md)

## Typescript 相关

- [Typescript: 深入 & 操作符与 extends 的区别](./articles/Typescript/Typescript深入&操作符与extends的区别.md)
- [深入 Typescript 中的 Enums](./articles/Typescript/深入Typescript中的Enums.md)
- [Utility Types in TypeScript](./articles/Typescript/Utility_Types_in_TypeScript.md)

## 算法相关

- [并查集 Disjoint Set](./articles/Algorithm/并查集Disjoint_Set.md)

## React 相关

- [初探React设计思路](./articles/React/初探React设计思路.md)

### hooks

- [intro](./articles/React/hooks/intro.md)
- [useState](./articles/React/hooks/useState.md)
- [useEffect](./articles/React/hooks/useEffect.md)
- [useContext](./articles/React/hooks/useContext.md)
- [useReducer](./articles/React/hooks/useReducer.md)
- [useCallback](./articles/React/hooks/useCallback.md)
- [useMemo](./articles/React/hooks/useMemo.md)
- [useRef](./articles/React/hooks/useRef.md)
- [useImperativeHandle](./articles/React/hooks/useImperativeHandle.md)
- [useLayoutEffect](./articles/React/hooks/useLayoutEffect.md)
- [useDebugValue](./articles/React/hooks/useDebugValue.md)
- [useTransition](./articles/React/hooks/useTransition.md)
- [useDeferredValue](./articles/React/hooks/useDeferredValue.md)
- [useId](./articles/React/hooks/useId.md)

## 基础相关

- [Computer Network](./articles/Basic/Computer_Network.md)

